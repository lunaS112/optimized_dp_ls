...
grid_resolution = np.array([15, 15, 10, 15, 15, 10])  

# Define the state space: (x1, y1, theta1, x2, y2, theta2)
g = Grid(
    np.array([-4, -4, -math.pi, -4, -4, -math.pi]),  # min values
    np.array([4, 4, math.pi, 4, 4, math.pi]),  # max values
    6, # number of dimensions
    grid_resolution,  # grid resolution
    [2, 5]  # Periodicity in theta1 and theta2
)

def EuclideanNormShape(grid: Grid, center: List, radius: float) -> np.ndarray:
    data = np.zeros(grid.pts_each_dim)
    data += np.power(grid.vs[0] - grid.vs[3], 2) # (x1-x2)^2
    data += np.power(grid.vs[1] - grid.vs[4], 2) # (y1-y2)^2

    return data - radius**2

# Define the initial value function
collision_radius = 1
Initial_value_f = EuclideanNormShape(g, np.zeros(6), collision_radius)
...
"""
class DubinsTwoCar:
    def __init__(self, speed=1.0, wMax=1.0, uMode="max", dMode="min"):
        self.speed = speed  # Fixed speed for both cars
    ...
    def dynamics(self, t, state, uOpt, dOpt):
        """
        Computes the dynamics of the two-car system.
        :param state: (x1, y1, theta1, x2, y2, theta2)
        :param uOpt: Control input for ego car (turning rate w1)
        :param dOpt: Disturbance input for adversary car (turning rate w2)
        ...
        x1_dot[0] = self.speed * hcl.cos(state[2])  # Ego car movement in x
        y1_dot[0] = self.speed * hcl.sin(state[2])  # Ego car movement in y
        theta1_dot[0] = uOpt[0]  # Ego car turning rate

        x2_dot[0] = self.speed * hcl.cos(state[5])  # Adversary movement in x
        y2_dot[0] = self.speed * hcl.sin(state[5])  # Adversary movement in y
        theta2_dot[0] = dOpt[0]  # Adversary turning rate
    ...
    def opt_ctrl(self, t, state, spat_deriv):
        ...
        return (opt_w1[0], in2[0], in3[0], in4[0])
    ...
    def opt_dstb(self, t, state, spat_deriv):
        ...
        return (opt_w2[0],  in2[0], in3[0], in4[0])
...
po = PlotOptions(
    ...
    slicesCut=[10,10,6],  
    # slicesCut=[math.floor(grid_resolution[3] / 2), math.floor(grid_resolution[4] / 2), math.floor(grid_resolution[5] / 2)],
    save_fig=True,
    filename="collision_avoidance.png"
)
